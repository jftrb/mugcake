...
INSERT INTO recipes (user_id, favorite, title, url, image_source, prep_info, tags, ingredient_sections, directions, notes)
	VALUES (
		(SELECT id from users where users.email = 'jeanfelix1@hotmail.fr'),
		true,
		...
		ARRAY (
			SELECT tags.id
			FROM	 unnest(ARRAY['Dessert', 'Easy', 'Tag 2', 'Tag 2', 'Really Long Tag 3']) WITH ORDINALITY AS a(tag_name, ord)
			JOIN   tags ON tags.name = a.tag_name
			ORDER  BY a.ord
		),
		'[{"header":"","ingredients":[{"quantity":3,"unit":"c. à soupe","ingredient":"de farine tout usage non blanchie"},{"quantity":2,"unit":"c. à soupe","ingredient":"de cassonade"},{"quantity":2,"unit":"c. à thé","ingredient":"de cacao"},{"quantity":0.25,"unit":"c. à thé","ingredient":"de poudre à pâte"},{"quantity":3,"unit":"c. à soupe","ingredient":"de lait"},{"quantity":1,"unit":"c. à soupe","ingredient":"d''huile de canola"},{"quantity":1,"unit":"goutte","ingredient":"d''extrait de vanille"},{"quantity":10,"unit":"pépites","ingredient":"de chocolat mi-sucré"}]}]',
...
FROM   recipes r WHERE r.user_id = (SELECT id from users where users.email = 'jeanfelix1@hotmail.fr');
